...
class PriorityQueue:
    ...
    def append(self, key):
        ...
        while current != 1 and self.queue[current] < self.queue[parent(current)]:
            swap(self.queue, current, parent(current))
    ...
    def pop(self):
        ...
        if len(self.queue) == 1:
            return None
        swap(self.queue, 1, len(self.queue)-1)
        key = self.queue.pop()
        x = 1
        while x < len(self.queue):
            smallest = x
            if left(x) < len(self.queue) and self.queue[left(x)] < self.queue[smallest]:
                smallest = left(x)
            if right(x) < len(self.queue) and self.queue[right(x)] < self.queue[smallest]:
                smallest = right(x)
            if smallest != x:
                swap(self.queue, x, smallest)
                x = smallest
            else:
                break
        return key
